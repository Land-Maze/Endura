...
private:
	...
	vk::SurfaceFormatKHR chooseSwapSurfaceFormat(const std::vector<vk::SurfaceFormatKHR>& availableFormats) {
		for (const auto& availableFormat : availableFormats) {
			std::print("Found available format: {} {}\n", to_string(availableFormat.format), to_string(availableFormat.colorSpace));
	...
	void createLogicalDevice() {
	    ...
	    graphicsFamilyIndex = static_cast<uint32_t>( std::distance( queueFamilyProperties.begin(), graphicsQueueFamilyProperty ) );
	    ...
	    if ( ( graphicsFamilyIndex == queueFamilyProperties.size() ) || ( presentFamilyIndex == queueFamilyProperties.size() ) )
	    {
	    ...
	    vk::DeviceCreateInfo      deviceCreateInfo{ .pNext =  &features, .queueCreateInfoCount = 1, .pQueueCreateInfos = &deviceQueueCreateInfo, .enabledExtensionCount = static_cast<uint32_t>( deviceExtensions.size()), .ppEnabledExtensionNames = deviceExtensions.data() };
